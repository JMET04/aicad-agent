...
9. Before compile, author `aicad_architectural_detail_contract_v2`, call `aicad_validate_architecture_detail_contract` (or `scripts/aicad_architecture_detail_qa.py`), and stop with `blocker_report_only` on failure. The contract must prove complete axis groups, the room equipment matrix, semantic interior layers, four dimension purposes, and door host-opening-sweep topology.
...
13. If the user requests construction-ready, fabrication-ready, manufacturing-ready or production-ready output, read `rules/production_readiness_rules.json` and validate a `rules/production_readiness_contract_v2.schema.json` contract with `scripts/aicad_production_readiness_qa_v2.py`. With `strictProductionOnly=true`, any failed gate exposes only the blocker report; more annotations may not compensate for missing engineering authority, host persistence or authorized release.
