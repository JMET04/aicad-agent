...
9. Before compile, author `aicad_architectural_detail_contract_v1`, call `aicad_validate_architecture_detail_contract` (or `scripts/aicad_architecture_detail_qa.py`), and stop with `blocker_report_only` on failure. The contract must prove complete axis groups, the room equipment matrix, semantic interior layers, four dimension purposes, and door host-opening-sweep topology.
