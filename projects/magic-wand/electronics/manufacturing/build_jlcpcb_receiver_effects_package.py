#!/usr/bin/env python3
"""Build and verify the JLCPCB bare-PCB package for the receiver-effects board.

This is a media-only receiver board that is ordered separately from the frozen
magic-wand board. It packages Gerber + drill only; BOM/CPL are copied for a
later PCBA sourcing pass and do not gate the bare-PCB upload.
"""
...
KICAD_CLI = Path(os.environ.get(
    "MAGIC_WAND_KICAD_CLI",
    r"C:\Users\刘佳明\AppData\Local\Programs\KiCad\10.0\bin\kicad-cli.exe",
))
...
def main() -> int:
    ...
    upload_drills = [DRILL / "receiver-effects-PTH.drl", DRILL / "receiver-effects-NPTH.drl"]
