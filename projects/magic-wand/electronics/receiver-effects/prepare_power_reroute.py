#!/usr/bin/env python3
"""Prepare a constrained power reroute around the accepted signal routing.

All accepted signal routes remain in the Specctra input.  Contract-width power
tracks are removed, except for at most 0.80 mm of 0.25 mm breakout from the
fine-pitch USB/radio/regulator/amplifier pads.  Freerouting then sees only the
power ratsnest while preserving the signal topology.
"""
...
def main() -> int:
    ...
    # Saving and reloading releases SWIG references held by removed tracks.
    # Removing zones from this same in-memory board after the track pass can
    # otherwise leave the Python wrapper with invalid object ownership.
    args.route_board.parent.mkdir(parents=True, exist_ok=True)
    if not pcbnew.SaveBoard(str(args.route_board.resolve()), route):
        raise RuntimeError("failed to save intermediate power-route board")
    route = pcbnew.LoadBoard(str(args.route_board.resolve(strict=True)))
