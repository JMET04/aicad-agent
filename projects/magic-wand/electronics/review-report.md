...
|---|---|---|---|
| BLK-EDA-001 | Blocker | Manufacturing qualification incomplete: receiver-effects A1 board passes native ERC 0/0 and DRC 0/0/0, but BOM/CPL are not fully LCSC-qualified, PCBA sourcing is not closed, and the NINA In1 ground coverage 98% production gate remains open (96% measured). | Complete BOM/LCSC/AVL review, close NINA coverage to 98%, and independently review the A1 CAM package before any PCBA order. |
| ENV-KICAD-001 | Environment note | KiCad 10.0.5 is installed and used (2026-08-23): receiver-effects A1 native ERC 0/0 and DRC 0/0/0 replayed via kicad-cli. `ngspice` and `arm-none-eabi-gcc` remain absent; winget is absent. | ngspice simulation, nRF target compile and hardware bring-up still required (see BLK-SIM-001 / BLK-FW-001). |
