...
& $PythonExe (Join-Path $root 'tools\aicad.py') compile (Join-Path $root 'examples\protocol3-text-layer.plan.json') --out $outputDirectory --name protocol3-text-layer
if ($LASTEXITCODE -ne 0) {
...
try {
    ...
    Invoke-AutoCadCoreTest -InputDrawing (Join-Path $testRootResolved 'input.dxf') -Script (Join-Path $testRootResolved 'run-v3.scr') -LogPrefix 'v3'
    $v3 = Get-Content -LiteralPath (Join-Path $testRootResolved 'v3-report.txt')
    if ($v3 -notcontains 'AICAD_V3_PASS') {
        throw "AutoCAD protocol 3 text/layer check failed: $($v3 -join '; ')"
    }
    Invoke-AutoCadCoreTest -InputDrawing (Join-Path $testRootResolved 'result-v3.dwg') -Script (Join-Path $testRootResolved 'run-v3-persistence.scr') -LogPrefix 'v3-persistence'
    $v3Persistence = Get-Content -LiteralPath (Join-Path $testRootResolved 'v3-persistence-report.txt')
