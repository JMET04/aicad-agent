...
def _validate_learning_runtime_behavior(plugin_root: Path, errors: list[str]) -> None:
    ...
    try:
        for surface, function_name, fields_name, schema in (
            ...
            invoke = (
                (lambda value: function(plugin_root, value, current_version="1.16.0"))
...
def verify(root: Path, source_root: Path | None = None) -> dict:
    ...
    if workflow.is_file():
        ...
        if "Version 1.16.0" not in workflow_text:
            errors.append("ci-not-pinned-to-1.16.0")
    ...
    archive = root / "dist" / "aicad-agent-1.16.0.zip"
