...
window.__aicadUi={get selectedRefs(){return selectedRefs},get operations(){return operations},get instructions(){return instructions},get coordinateVisible(){return coordinateVisible},get activeStorey(){return activeStorey},toggleSelectionRef,selectParameter,formalCorrection,handoff,renderUi,setCoordinateVisible,setActiveStorey};
"""
...
def render_review_html_v2(package: dict[str, Any], selector_script: str = "") -> str:
    ...
    for view in package["views"]:
        ...
        card_class = "view-card storey-view" if managed else "view-card"
...
main{{display:grid;grid-template-columns:minmax(0,1fr) 390px;gap:14px;padding:14px;align-items:start}}.workspace-shell{{min-width:0}}.workspace{{display:grid;grid-template-columns:repeat(2,minmax(320px,1fr));gap:12px}}.storey-switcher{{display:flex;align-items:center;gap:7px;margin:0 0 10px;padding:8px 10px;background:var(--panel);border:1px solid var(--line)}}.storey-switcher>span{{font-size:11px;color:var(--muted);margin-right:3px}}.storey-tab{{padding:6px 13px;font-weight:700}}.storey-tab.active,.storey-tab[aria-pressed="true"]{{background:var(--navy);border-color:var(--navy);color:#fff}}[hidden]{{display:none!important}}.view-card,.inspector{{background:var(--panel);border:1px solid var(--line);box-shadow:3px 3px 0 #27374618}}.view-card{{min-height:292px;padding:12px}}.storey-view{{grid-column:1/-1;min-height:1160px}}.storey-view .cad-view{{height:1100px}}.selector-card,.section-card{{grid-column:span 2}}.view-heading{{display:flex;justify-content:space-between;gap:12px;align-items:end;margin-bottom:9px;border-bottom:1px solid #d9d5ca;padding-bottom:7px}}.view-heading h2{{margin:1px 0 0;font-size:15px}}.eyebrow{{font:700 9px Consolas;color:var(--orange);letter-spacing:.12em}}.hint{{font-size:11px;color:var(--muted);text-align:right}}
